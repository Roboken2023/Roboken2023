...
float kp{0.0011}, ki{0.03}, kd{0.006}, last_error{0};
...
void turnRight(int target_angle){
  digitalWrite(A7, HIGH);
  setupIMU();
  getAngle();
  target_angle = target_angle+angle;
  while(abs(target_angle - angle) > 1){
    left(200);
    getAngle();
...
void turnLeft(int target_angle){
  digitalWrite(A7, HIGH);
  setupIMU();
  getAngle();
  target_angle = -target_angle+angle;
  while(abs(target_angle + abs(angle)) > 1){
   right(200);
    getAngle();
